...
```{r}
mydata <- load_sav_files("data/raw/raw_S2_200624")
...
mydata <- lapply(mydata, remove_duplicates)

mydata <- mydata$StudentChatGPT_T1.sav %>%
  left_join(., mydata$StudentChatGPT_T2.sav
            , by = "PROLIFIC_PID") %>%
  left_join(., mydata$StudentChatGPT_T3.sav
            , by = "PROLIFIC_PID") %>%  
  left_join(., mydata$StudentChatGPT_T4.sav
            , by = "PROLIFIC_PID") %>%
  left_join(., mydata$StudentChatGPT_T5.sav
            , by = "PROLIFIC_PID") 
```
...
saveRDS(mydata, "data/documented/meta_data_S2.rds")

writexl::write_xlsx(mydata, "data/documented/mydata_S2.xlsx")
...
df_wide <- readxl::read_excel("data/documented/mydata_S2.xlsx")
...
df <- df %>%  
  mutate(b1socinfnew = rowMeans(dplyr::select(., t2socinf_1, t2socinf_2), na.rm = TRUE)) %>% 
  mutate(b1facilitatnew = rowMeans(dplyr::select(., t2facilitat_1, t2facilitat_2), na.rm = TRUE))
...
```{r}
# tooluse columns
df_wide <- df %>% transform_toolused01_s2(.)
  
df <- transform_entries_toolused2(df_wide) # transform tool entries to unique tool names
...
## Tools used
df$toolsused <- paste(df$t1tooluse, ",", df$t2tooluse, ",", df$t3tooluse, ",",
                      df$t4tooluse, ",", df$t5tooluse)
...
name_vector <- unlist(strsplit(df$uniquetoolsused, ",")) %>% trimws(.)

tool_vector <- refinr::key_collision_merge(
name_vector,
ignore_strings = NULL,
bus_suffix = TRUE,
dict = NULL
...
tool_vector <- sub("a section of a research paper in preparation for journal club", "Other", tool_vector) 
tool_vector <- sub("ChatGPTCOPILOT", "ChatGPT, Copilot", tool_vector)
tool_vector <- sub("Bingai", "Bing", tool_vector)
tool_vector <- sub("JasperAI", "Jasper", tool_vector)
tool_vector <- sub("meta", "MetaAI", tool_vector)
tool_vector <- sub("Microsoft Bing", "Bing", tool_vector)
tool_vector <- sub("Qestion AI", "Question AI", tool_vector)
tool_vector <- sub("Quibolt", "Quillbot", tool_vector)
tool_vector <- sub("Quilbot", "Quillbot", tool_vector)
tool_vector <- sub("Used ChatGPTto today to help me underst", "ChatGPT", tool_vector)
tool_vector <- sub("ChatGPTBingai", "ChatGPT, Bing", tool_vector)
tool_vector <- sub("ChatGPTBing", "ChatGPT, Bing", tool_vector)
tool_vector <- sub("ChatGPTMetaAI Capilot", "ChatGPT, MetaAI, Copilot", tool_vector)
tool_vector <- unlist(strsplit(tool_vector, ",")) %>% trimws(.)


sort(unique(tool_vector))
...
# Values to assign 1
subjects_to_match <- c("Engineering", "Medicine & Health", "Natural Sciences", 
...
## Training 
df <- concat_values_training_1(df)
...
df <- df %>%
  mutate(t1country_4_text = case_when(
    t1country == 1 ~ "Germany",
    t1country == 2 ~ "Austria",
    t1country == 3 ~ "Switzerland",
    TRUE ~ t1country_4_text
  ))
...
df <- df %>%
  ...
  mutate(t1country_4_text = case_when(t1country_4_text == "ENGLAND" ~ "United Kingdom",
                               t1country_4_text == "england" ~ "United Kingdom",
                               t1country_4_text == "SA" ~ "South Africa",
                               t1country_4_text == "Soth Africa" ~ "South Africa",
                               t1country_4_text == "South Afrıca" ~ "South Africa",
                               t1country_4_text == "Wales" ~ "United Kingdom",
                             # otherwise, leave it as is
                             TRUE ~ t1country_4_text)) %>%
  # standardize country names to iso3c and the countrycode country names
  mutate(t1country = countryname(df$t1country_4_text)) 
...
df$t1degree <- factor(df$t1degree, labels = c("Bachelor", "Master", "PhD"))
...
# reverse code facilitating conditions 3
df <-  df  %>% 
    mutate(t2facilitat_3 = 8 - t2facilitat_3)
```

```{r}
# Update the timespent columns
df <- update_timespent(df, "t", "t", 5)

df <- df %>%
  filter(nchar(id.x) >= 20)
...
cols <- c("t2tooluse", "t3tooluse", "t4tooluse", "t5tooluse")
...
df <- df %>%
      mutate(t2useful_1 = if_else(t2tooluse == "Not used", NA, t2useful_1, missing = t2useful_1)) %>%
      mutate(t3useful_1 = if_else(t3tooluse == "Not used", NA, t3useful_1, missing = t3useful_1)) %>%
      mutate(t4useful_1 = if_else(t4tooluse == "Not used", NA, t4useful_1, missing = t4useful_1)) %>%
      mutate(t5useful_1 = if_else(t5tooluse == "Not used", NA, t5useful_1, missing = t5useful_1)) 
```
## Remove if not used at T1

```{r}
# Define the columns of interest
cols <- c("t1useful_1")

# Use dplyr to filter out rows where more than 3 of the specified columns are NA
df_in <- df %>%
  filter(rowSums(is.na(select(., all_of(cols)))) < 1)

cat("Not participated in T1:", nrow(df) - nrow(df_in))
...
df <- df %>%
  mutate(failed_checks = case_when(t1anx_8 != 3 ~ 1,
                             # otherwise, leave it as is
                             TRUE ~ failed_checks)) 

sum(df$failed_checks[df$failed_checks == 1], na.rm = T)

df <- df %>% 
  filter(failed_checks < 1) %>%
  dplyr::select(-matches("t1anx_8|failed_checks|commitment"))

```

## Remove if only one observation

```{r}
# Define the columns of interest
cols <- c("t1useful_1", "t2useful_1", "t3useful_1", "t4useful_1", "t5useful_1")

# Use dplyr to filter out rows where more than 3 of the specified columns are NA
df_in <- df %>%
  filter(rowSums(is.na(select(., all_of(cols)))) <= 3)

# Define the columns of interest
cols <- c("t2facilitat_1")
...
cat("Participated in less than two surveys:", nrow(df) - nrow(df_in))

df <- df_in

```

## Prepare longer data frame

```{r}
for (col in names(df)[which(names(df) == 't1tool'):which(names(df) == 't1experience')]){
    names(df)[names(df) == col] <-  sub("^t", "b", col)
}

for (col in names(df)[which(names(df) == 't1receivetr'):which(names(df) == 't1wishtr')]){
    names(df)[names(df) == col] <-  sub("t", "b", col)
}
...
longer <- df %>% dplyr::select_if(., is.numeric) %>% 
  dplyr::select(-matches("failed_checks|t[1-8]id|commitment|prolific_pid")) %>% 
 pivot_longer(matches("t1|t2|t3|t4|t5"),
    names_pattern = "(^t.)(.*)",
...
longer <- longer %>%
      group_by(id) %>%
      fill(starts_with("socinf_"), .direction = "downup")


longer <- longer %>%
      group_by(id) %>%
      fill(starts_with("gender_binary"), .direction = "downup")
...
longer <- longer %>%
      group_by(id) %>%
      fill(starts_with("facilitat_"), .direction = "downup")
...
names(longer) <- mgsub::mgsub(names(longer), 
                          c("b1anx_1$", "b1anx_3", "b1anx_5", "b1anx_6", "b1anx_7", "b1anx_9", 
                            "b1anx_10", "b1anx_11"), 
                          c("b1anx_1", "b1anxout_3", "b1anxout_5", "b1anxout_6", "b1anxout_7", "anxout_9",
                            "b1anxout_10", "b1anxout_11"))

names(df) <- mgsub::mgsub(names(df), 
                          c("t2facilitat_3$", "t2facilitat_4"), 
                          c("t2facilitatout_3", "t2facilitatout_4"))

names(longer) <- mgsub::mgsub(names(longer), 
                          c("facilitat_3$", "facilitat_4"), 
                          c("facilitatout_3", "facilitatout_4"))

names(df) <- mgsub::mgsub(names(df), 
                          c("t2socinf_3$", "t2socinf_4"), 
                          c("t2socinfout_3", "t2socinfout_4"))

names(longer) <- mgsub::mgsub(names(longer), 
                          c("socinf_3$", "socinf_4"), 
                          c("socinfout_3", "socinfout_4"))
```
...
df_oneitem <- df %>% dplyr::select_if(., is.numeric) %>% dplyr::select(-matches("socinf|facilitat|anx|reliable|pace|useful|ease|strain|over|gopro|id$"))

comp_split <- df %>% dplyr::select_if(., is.numeric) %>% dplyr::select(matches("socinf|facilitat|anx|reliable|pace|useful|ease|strain|over|gopro")) %>% 
  split.default(sub("_\\d{1,2}$", "", names(.))) 

alph_split <- df %>% dplyr::select_if(., is.numeric) %>% dplyr::select(matches("socinf_|facilitat_|anx|reliable|pace|useful|ease|strain|over")) %>% 
  split.default(sub("_\\d{1,2}$", "", names(.))) 
...
comp_df <- do.call("cbind", comp) %>%
  cbind(df_oneitem, df_notnum, .) 
alph_df <- do.call("rbind", alph) %>% round(., 2)

round(splithalfr::spearman_brown(df$t2facilitat_1, df$t2facilitat_2), 2)
round(splithalfr::spearman_brown(df$t2socinf_1, df$t2socinf_2), 2)
...
# rename t1 to b1
exceptions <- comp_df %>% dplyr::select(matches("used|timespent|task|ease|over|pace|reliable|strain|useful")) %>% names(.)
comp_df <- rename_t1b1(comp_df, exceptions)

comp_df <- comp_df %>% rename(b1facilitat = t2facilitat,
                              b1socinf = t2socinf)
...
comp_df <- comp_df %>% 
  relocate(starts_with("b1"), .after = last_col()) %>%
...
longer_comp <- comp_df %>% 
 pivot_longer(matches("t1|t2|t3|t4|t5|t6"),
    names_pattern = "(t.)(.*)", 
   names_to = c( "set", ".value")
 ) %>% mutate(set = (as.numeric(textclean::mgsub(.$set, c("t", "_"), c("", "")))))


longer_comp$id <- rep(seq(1,nrow(comp_df)), each=5)
```
...
```{r}
writexl::write_xlsx(longer_comp, "data/preprocessed/longer_comp.xlsx")
writexl::write_xlsx(df, "data/preprocessed/df_wide.xlsx")
```
...
# create person means
variables <- c("ease", "useful", "timespent", "timespentahead", "pace", "reliable", "over", "strain", "strainahead", "noease", "nouseful", "nopace", "noreliable", "noover", "nostrain")
...
longer_comp <- calculate_CWC(longer_comp)

# create grand means
variables <- longer_comp %>% dplyr::select(matches("b1|f1|f2|wishtr|media|anx|facilitat|socinf")) %>% dplyr::select(where(is.numeric)) %>%  names(.)
longer_comp <- longer_comp %>% 
...
variables <- longer_comp %>% dplyr::select(matches("b1|f1|f2|wishtr|media|anx|extraversion|agreeableness|conscientiousness|neuroticism|openness|facilitat|socinf")) %>% dplyr::select(where(is.numeric)) %>%  names(.)
longer_comp <- longer_comp %>%
  mutate(across(all_of(variables), ~ . - mean(., na.rm = TRUE), .names = "{.col}.CGM"))

longer_comp$tooluse <- as.factor(longer_comp$used)

longer_comp$tooluseahead <- lead(longer_comp$used)

longer_comp$tooluse_num <- as.numeric(levels(longer_comp$tooluse))[longer_comp$tooluse]
longer_comp$tooluse_numahead <- longer_comp$tooluseahead
```

## Exclude NAs

```{r}
longer_comp_nona <- longer_comp %>% filter_at(vars(used), all_vars(!is.na(.))) 
```
...
round(sd(longer_comp_short_nona$b1age, na.rm = T), 2)
DescTools::PercTable(longer_comp_short_nona$b1gender)
sort(prop.table(table(longer_comp_short_nona$b1country)))

name_vector <- longer_comp_short_nona$b1country
...
name_freq <- sort(table(name_vector))
name_freq <- data.frame(name_freq) %>% 
  mutate(Percentage = paste0(100*round(Freq/sum(Freq),3), "%"),
         perc = Freq/sum(Freq)) %>% select(name_vector, Freq, Percentage)

## Study fields ##
stud_freq <- table(longer_comp_short_nona$b1studies) %>% data.frame(.)
stud_freq <- stud_freq[order(as.integer(stud_freq$Freq),decreasing = TRUE), ]
stud_freq$Var1 <- reorder(stud_freq$Var1, stud_freq$Freq)

stud_freq <- stud_freq %>% filter(Freq > 1)
stud_freq <- data.frame(stud_freq) %>% 
  mutate(Percentage = paste0(100*round(Freq/sum(Freq),3), "%"),
         perc = Freq/sum(Freq)) %>% select(Var1, Freq, Percentage)

coul <- RColorBrewer::brewer.pal(5, "Set2") 
barplot(height=stud_freq$Freq,  names=str_wrap(stud_freq$Var1, 12), col=coul, cex.names = 0.8)
...
name_freq <- data.frame(name_freq) %>% 
  mutate(Percentage = paste0(100*round(Freq/sum(Freq),2), "%"),
         perc = Freq/sum(Freq))
...
corrs <- as.data.frame(cortab) %>% cbind(ICC_2, .)

corrs <- corrs %>% 
  mutate(across(c(ease.wg, useful.wg, timespent.wg, b1anx.wg, b1anx.wg, b1facilitat.wg,
                  b1socinf.wg, b1age.wg, b1gender_binary.wg,
                  b1experience.wg, b1media.wg), ~ numformat(.)))


corrs <- rbind(corrs, c("", mean), c("", sd))

rownames(corrs) <- c("PEOU", "PU", "Usage", "Anxiety", "FC", "SN", "Age", "Gender", "Experience", "Media", "M", "SD")
colnames(corrs) <- c("ICC", "PEOU", "PU", "Usage", "Anxiety", "FC", "SN", "Age", "Gender", "Experience", "Media")
...
html_table %>% flextable::save_as_docx( path = "output/cortableS2.docx", pr_section = sect_properties)
```

## Prepare data for lagged relationships
...
longer <- 
    longer %>%
      group_by(id) %>% fill(starts_with("studies_tech"), .direction = "downup")  
...
```{r}
library(readxl)
longer_comp_short_nona_S1 <- read_excel("data/preprocessed/longer_comp_short_nona_S1.xlsx")
longer_comp_S1 <- read_excel("data/preprocessed/longer_comp_S1.xlsx")
...
mod <- t.test(longer_comp_short_nona$b1facilitat, not_include$b1facilitatnew, alternative = "two.sided", var.equal = FALSE)
sd(longer_comp_short_nona$b1anx)
report::report_statistics(mod)

mod <- t.test(longer_comp_short_nona$b1socinf, not_include$b1socinfnew, alternative = "two.sided", var.equal = FALSE)
sd(longer_comp_short_nona$b1anx)
report::report_statistics(mod)

# S1 VS S2
mod <- t.test(longer_comp_short_nona$b1anx, longer_comp_short_nona_S1$b1anx, alternative = "two.sided", var.equal = FALSE)
report::report_statistics(mod)
...
# S1 VS S2
mod <- t.test(longer_comp_short_nona$b1experience, longer_comp_short_nona_S1$b1experience, alternative = "two.sided", var.equal = FALSE)
report::report_statistics(mod)


studies_table <- rbind(table(longer_comp_short_nona$b1studies_tech), table(not_include$b1studiestech))
# Perform chi-square test
chisq_test <- chisq.test(studies_table, correct = F)
# Print the result
report::report(chisq_test)

studies_table <- rbind(table(longer_comp_short_nona$b1studies_tech), table(longer_comp_short_nona_S1$b1studiestech))
# Perform chi-square test
chisq_test <- chisq.test(studies_table, correct = F)
# Print the result
report::report(chisq_test)
...
mod <- t.test(longer_comp_short_nona$ease, not_include$t1ease, alternative = "two.sided", var.equal = FALSE)
sd(longer_comp_short_nona$ease)
sd(not_include$ease, na.rm = T)
report::report_statistics(mod)

# S1 VS S2
mod <- t.test(longer_comp_short_nona$ease, longer_comp_short_nona_S1$ease, alternative = "two.sided", var.equal = FALSE)
report::report_statistics(mod)

mod <- t.test(longer_comp_short_nona$useful, not_include$t1useful, alternative = "two.sided", var.equal = FALSE)
sd(longer_comp_short_nona$useful)
sd(not_include$useful, na.rm = T)
report::report_statistics(mod)

# S1 VS S2
mod <- t.test(longer_comp_short_nona$useful, longer_comp_short_nona_S1$useful, alternative = "two.sided", var.equal = FALSE)
report::report_statistics(mod)

# S1 VS S2
mod <- t.test(longer_comp_short_nona$timespent, longer_comp_short_nona_S1$timespent, alternative = "two.sided", var.equal = FALSE)
report::report_statistics(mod)
...
# S1 VS S2
mod <- t.test(longer_comp_short_nona$b1age, longer_comp_short_nona_S1$b1age, alternative = "two.sided", var.equal = FALSE)
report::report_statistics(mod)
...
# S1 VS S2
mod <- t.test(longer_comp_short_nona$b1media, longer_comp_short_nona_S1$b1media, alternative = "two.sided", var.equal = FALSE)
report::report_statistics(mod)
...
# S1 VS S2
gender_table <- rbind(table(longer_comp_short_nona$b1gender_binary), table(longer_comp_short_nona_S1$b1gender_binary))
# Perform chi-square test
chisq_test <- chisq.test(gender_table, correct = F)
# Print the result
report::report(chisq_test)
...
# Comparisons of variations Study 1 and Study 2

```{r}
cols <- intersect(names(longer_comp_S1), names(longer_comp))
longer_comp_comp_S1 <- longer_comp_S1[, cols]

longer_comp_comp_S1$set <- longer_comp_comp_S1$set -1
longer_comp_comp <- longer_comp[, cols]

longer_comp_comp_S1$group <- "Study 1"
longer_comp_comp$group <- "Study 2"

combined_data <- rbind(longer_comp_comp_S1, longer_comp_comp)
```
...
# RMSS Ease
rmss_ease_S1 <- longer_comp_comp_S1 %>% drop_na(ease) %>% 
mutate(rmss_ease = rmssd_id(.$ease, .$id)) %>% group_by(id) %>% summarize(rmss_ease = mean(rmss_ease)) 

round(mean(rmss_ease_S1$rmss_ease), 2)
round(sd(rmss_ease_S1$rmss_ease), 2)

rmss_ease_S2 <- longer_comp_comp %>% drop_na(ease) %>% 
mutate(rmss_ease = rmssd_id(.$ease, .$id)) %>% group_by(id) %>% summarize(rmss_ease = mean(rmss_ease)) 

round(mean(rmss_ease_S2$rmss_ease), 2)
round(sd(rmss_ease_S2$rmss_ease), 2)

rmss_ease_S1$group <- 1
rmss_ease_S2$group <- 2
combined_data_rmss <- rbind(rmss_ease_S1, rmss_ease_S2)

# Kruskall Test
library(rcompanion)
shapiro.test(combined_data_rmss$rmss_ease)
kruskal.test(rmss_ease ~ group, data = combined_data_rmss)


summary_data <- combined_data %>%
  group_by(group, set) %>%
  summarize(mean_score = mean(ease, na.rm = TRUE),
            sd_score = sd(ease, na.rm = TRUE))


# Calculate within-person variance for each participant
within_person_variance <- combined_data %>%
  group_by(id, group) %>%
  summarize(within_variance = var(ease, na.rm = TRUE)) %>%
  ungroup()

# Filter out near-zero and zero variances
within_person_variance <- within_person_variance %>%
  filter(within_variance > 1e-6)

# Create the plot to visualize within-person variance
plot_within_variance_ease <- ggplot(within_person_variance, aes(x = group, y = within_variance, color = group)) +
  geom_boxplot(outlier.shape = NA) +                       # Remove default outliers
  geom_point(size = 1, alpha = 0.6, 
             position = position_jitter(width = 0.2)) +    # Add individual points with jitter
  scale_y_log10(limits = c(0.01, 10), breaks = c(0.01, 0.1, 1, 10)) +  # Apply log transformation to y-axis and set limits
  labs(
       x = "Study",
       y = "Within-person variance PEOU (log scale)") +
  theme_minimal(base_size = 10) +                           # Use a minimal theme for cleaner look
  theme(
...
ggsave("output/Within_Person_Variance_ease.tiff", plot_within_variance_ease, width = 5, height = 3, dpi = 300)



# RMSS Useful
rmss_useful_S1 <- longer_comp_comp_S1 %>% drop_na(useful) %>% 
mutate(rmss_useful = rmssd_id(.$useful, .$id)) %>% group_by(id) %>% summarize(rmss_useful = mean(rmss_useful)) 

round(mean(rmss_useful_S1$rmss_useful), 2)
round(sd(rmss_useful_S1$rmss_useful), 2)

rmss_useful_S2<- longer_comp_comp %>% drop_na(useful) %>% 
mutate(rmss_useful = rmssd_id(.$useful, .$id)) %>% group_by(id) %>% summarize(rmss_useful = mean(rmss_useful)) 

round(mean(rmss_useful_S2$rmss_useful), 2)
round(sd(rmss_useful_S2$rmss_useful), 2)

summary_data <- combined_data %>%
  group_by(group, set) %>%
  summarize(mean_score = mean(useful, na.rm = TRUE),
            sd_score = sd(useful, na.rm = TRUE))


rmss_useful_S1$group <- 1
rmss_useful_S2$group <- 2
combined_data_rmss <- rbind(rmss_useful_S1, rmss_useful_S2)

# Kruskall Test
library(rcompanion)
shapiro.test(combined_data_rmss$rmss_useful)
kruskal.test(rmss_useful ~ group, data = combined_data_rmss)

# Calculate within-person variance for each participant
within_person_variance <- combined_data %>%
  group_by(id, group) %>%
  summarize(within_variance = var(useful, na.rm = TRUE)) %>%
  ungroup()
...
# Create the plot to visualize within-person variance
plot_within_variance_useful <- ggplot(within_person_variance, aes(x = group, y = within_variance, color = group)) +
  geom_boxplot(outlier.shape = NA) +                       # Remove default outliers
  geom_point(size = 1, alpha = 0.6, 
             position = position_jitter(width = 0.2)) +    # Add individual points with jitter
  scale_y_log10(limits = c(0.01, 10), breaks = c(0.01, 0.1, 1, 10)) +  # Apply log transformation to y-axis and set limits
  labs(
       x = "Study",
       y = "Within-person variance PU (log scale)") +
  theme_minimal(base_size = 10) +                           # Use a minimal theme for cleaner look
  theme(
    text = element_text(size = 8),     
    axis.text = element_text(size = 8), 
...
# Save the plot as a TIFF file
ggsave("output/Within_Person_Variance_useful.tiff", plot_within_variance_useful, width = 5, height = 3, dpi = 300)


# RMSS Timespent
rmss_timespent_S1 <- longer_comp_comp_S1 %>% drop_na(timespent) %>% 
mutate(rmss_timespent = rmssd_id(.$timespent, .$id)) %>% group_by(id) %>% summarize(rmss_timespent = mean(rmss_timespent)) 

round(mean(rmss_timespent_S1$rmss_timespent), 2)
round(sd(rmss_timespent_S1$rmss_timespent), 2)

rmss_timespent_S2<- longer_comp_comp %>% drop_na(timespent) %>% 
mutate(rmss_timespent = rmssd_id(.$timespent, .$id)) %>% group_by(id) %>% summarize(rmss_timespent = mean(rmss_timespent)) 

round(mean(rmss_timespent_S2$rmss_timespent), 2)
round(sd(rmss_timespent_S2$rmss_timespent), 2)

summary_data <- combined_data %>%
  group_by(group, set) %>%
  summarize(mean_score = mean(timespent, na.rm = TRUE),
            sd_score = sd(timespent, na.rm = TRUE))

rmss_timespent_S1$group <- 1
rmss_timespent_S2$group <- 2
combined_data_rmss <- rbind(rmss_timespent_S1, rmss_timespent_S2)

# Kruskall Test
library(rcompanion)
shapiro.test(combined_data_rmss$rmss_timespent)
kruskal.test(rmss_timespent ~ group, data = combined_data_rmss)

# Calculate within-person variance for each participant
within_person_variance <- combined_data %>%
  group_by(id, group) %>%
  summarize(within_variance = var(timespent, na.rm = TRUE)) %>%
  ungroup()
...
# Create the plot to visualize within-person variance
plot_within_variance_timespent <- ggplot(within_person_variance, aes(x = group, y = within_variance, color = group)) +
  geom_boxplot(outlier.shape = NA) +                       # Remove default outliers
  geom_point(size = 1, alpha = 0.6, 
             position = position_jitter(width = 0.2)) +    # Add individual points with jitter
  scale_y_log10(limits = c(0.01, 10), breaks = c(0.01, 0.1, 1, 10)) +  # Apply log transformation to y-axis and set limits
  labs(
       x = "Study",
       y = "Within-person variance Usage (log scale)") +
  theme_minimal(base_size = 10) +                           # Use a minimal theme for cleaner look
  theme(
    text = element_text(size = 8), 
    axis.text = element_text(size = 8), 
...
# Save the plot as a TIFF file
ggsave("output/Within_Person_Variance_usage.tiff", plot_within_variance_timespent, width = 5, height = 3, dpi = 300)
...
useful <-lmer(useful~1+(1|id),data=longer_comp_nona)
...
model <- '
    ...
    level: 2
        ...
        b_facilitat =~  facilitat_1 + facilitat_2 
        b_socinf =~  socinf_1 + socinf_2 
'

library(lavaan)
fit <- sem(model, data = longer, cluster = "id", std.lv  = TRUE, verbose = FALSE)
...
model <- '
    ...
    level: 2
        ...
        ease_b =~ ease_1 + ease_2 + ease_3 + ease_4 + ease_5 + ease_6
        b_anx =~  b1anx_2 + b1anx_4 + b1anx_12
        b_facilitat =~  facilitat_1 + facilitat_2 + socinf_1 + socinf_2
'
...
## Perceived ease of use and perceived usefulness serially mediate a) the negative relationship of AI chatbot anxiety and b) the positive relationship of facilitating conditions with the daily time spent using AI chatbots. 
...
model1 <- '
        ...
        # direct effect
        b_timespent ~ bc2*b_anx + bc3*b_facilitat + sc*b_socinf
        b_ease ~ ba2*b_anx + ba3*b_facilitat + b_socinf + b1age + b1gender + b1experience + b1media
        
        # mediators
        b_useful ~  b_anx + b_facilitat + sa*b_socinf + bb_m1*b_ease + b1age + b1gender + b1experience + b1media 
        ...
        facil_useful_between := ba3*bb_m1
        facil_useful_within := ba3*wb_m1
...
semTools::monteCarloCI(fit1, nRep = 50000, standardized = F)
...
x <- c("anx_useful_between", "facil_useful_between", "ease_usage_between", "ba2bb", "ba3bb", "bsabb",
       "anx_useful_within", "facil_useful_within", "ease_usage_within", "ba2wb", "ba3wb", "bsawb")
...
mediation_paths %>% flextable::save_as_docx( path = "output/indirect2.docx")
...
pars.regressions %>% flextable::save_as_docx( path = "output/coefficients2.docx")
...
## H5: Perceived ease of use and perceived usefulness serially mediate a) the negative relationship of NLP chatbot anxiety and b) the positive relationship of facilitating conditions with the next-day time spent using AI chatbots. 

## H6: Perceived usefulness mediates the positive relationship of social influence with the next-day time spent using AI chatbots.
...
model1 <- '
        ...
        # direct effect
        b_timespent ~ bc2*b_anx + bc3*b_facilitat 
        b_ease ~ ba2*b_anx + ba3*b_facilitat 
        
        # mediators
        b_useful ~ bb_m1*b_ease + sa*b_socinf + b_anx + b_facilitat 
        b_timespent ~ bb_m2*b_useful + b1age + b1gender + b1media + sc*b_socinf
...
## H7: Experience moderates:

### H7a: the negative relationship of AI chatbot anxiety with perceived ease of use.

### H7b: the positive relationship of ease of use with perceived usefulness.

### H7c: the positive relationship of social influence with perceived usefulness.


```{r}
longer <- semTools::indProd(longer, var1=6:16, var2=2, match = FALSE, meanC = TRUE, residualC = FALSE, doubleMC = FALSE, namesProd = NULL) # (b)anxiety*experience
longer <- semTools::indProd(longer, var1=63:66, var2=2, match = FALSE, meanC = TRUE, residualC = FALSE, doubleMC = FALSE, namesProd = NULL) # (b)facilitat*experience
longer <- semTools::indProd(longer, var1=59:62, var2=2, match = FALSE, meanC = TRUE, residualC = FALSE, doubleMC = FALSE, namesProd = NULL) # (b)socinf*experience
longer <- semTools::indProd(longer, var1=39:44, var2=2, match = FALSE, meanC = TRUE, residualC = FALSE, doubleMC = FALSE, namesProd = NULL) # (b/w)ease*experience
```
...
model1 <- '
        ...
        b_facilitatexp =~ facilitat_1.b1experience + facilitat_2.b1experience +
        facilitat_3.b1experience + facilitat_4.b1experience 
        
        b_socinfexp =~ socinf_1.b1experience + socinf_2.b1experience + 
        socinf_3.b1experience + socinf_4.b1experience 
        ...
        facilitat_1 ~~ facilitat_1.b1experience
        facilitat_2 ~~ facilitat_2.b1experience
        facilitat_3 ~~ facilitat_3.b1experience
        facilitat_4 ~~ facilitat_4.b1experience
        
        socinf_1 ~~ socinf_1.b1experience
        socinf_2 ~~ socinf_2.b1experience
        socinf_3 ~~ socinf_3.b1experience
        socinf_4 ~~ socinf_4.b1experience
        
        
        # direct effect
        b_timespent ~ bc2*b_anx + bc3*b_facilitat + b1age + b1gender + b1media + b_experience
        b_useful ~ b_anx + b_facilitat + b_ease + b_experience
        
        # mediator effects
        b_ease ~ ba2*b_anxexp + ba3*b_facilitatexp + b_anx + b_facilitat + b_experience
        b_useful ~ bb_m1*b_easeexp + sa*b_socinfexp +  b_socinf
        b_timespent ~ bb_m2*b_useful  + sc*b_socinf 
...
model1 <- '
        ...
        b_timespent ~ bb_m2*b_useful + b1age + b1gender + b1media + studies_tech 
...
```{r}
# Extract the parameter estimates dataframe
pe <- parameterestimates(fit1, standardized = T)

# Select only the rows where the operation is a regression ("~")
regression_coefficients <- pe %>%
  filter(op == "~") %>%
  select(lhs, rhs, est, se, ci.lower, ci.upper, pvalue) %>% 
  mutate_if(is.numeric, round, digits = 3)
...
# Re-extracting the parameter estimates dataframe
pe <- standardizedSolution(fit1)

# Filter rows where the paths are part of the mediation model (including indirect effect)
mediation_paths <-  pe %>%
  filter(op == ":=") %>%
  select(lhs, rhs, est.std, se, ci.lower, ci.upper, pvalue) %>% 
  mutate_if(is.numeric, round, digits = 3) %>% 
  mutate(CI = paste0("[",ci.lower, ", ", ci.upper, "]")) %>% 
  select(lhs, est.std, se, CI, pvalue)

# Create a flextable object
mediation_paths <- flextable::flextable(mediation_paths)

mediation_paths %>% flextable::save_as_docx( path = "output/indirect1.docx")

```

## H1: Perceived ease of use mediates a) the negative relationship of AI chatbot anxiety and b) the positive relationship of facilitating conditions with daily perceived usefulness of AI chatbots.
...
model <- '
        ...
        # direct effect
        b_useful ~ bc2*b_anx + bc3*b_facilitat  + b1age + b1gender + b1media
        b_ease ~  ba2*b_anx + ba3*b_facilitat + b1age + b1gender + b1media
...
### Probing anxiety effect 

#### Indirect effect through ease of use
...
model_indir <- '
        ...
        # direct effect
        b_useful ~ bc2*b_anx + bc3*b_facilitat + si*b_socinf + b1age + b1gender + b1media
        b_ease ~  ba2*b_anx + ba3*b_facilitat + b1age + b1gender + b1media
...
fit_indir <- sem(model = model_indir, data = longer, cluster = "id")
summary(fit_indir, std=T, fit.measures = TRUE)
...
model_dir <- '
        ...
        # direct effect
        b_useful ~ bc2*b_anx + bc3*b_facilitat + si*b_socinf + b1age + b1gender + b1media
        b_ease ~ ba3*b_facilitat + b1age + b1gender + b1media
...
fit_dir <- sem(model = model_dir, data = longer, cluster = "id")
summary(fit_dir, std=T, fit.measures = TRUE)
    
```

```{r}
anova(fit_dir, fit_indir)
```

### Probing facilitating conditions
...
```{r}
anova(fit_si_dir, fit_si_indir)
```

## H2: Social influence is directly related to daily perceived usefulness. 
...
model_si_indir <- '
        ...
        # direct effect
        b_useful ~ bc2*b_anx + bc3*b_facilitat + bc4*b_socinf + b1age + b1gender + b1media
        b_ease ~  ba2*b_anx + ba3*b_facilitat + ba4*b_socinf + b1age + b1gender + b1media
...
fit_si_indir <- sem(model = model_si_indir, data = longer, cluster = "id")
summary(fit_si_indir, std=T, fit.measures = TRUE)
...
model_si_dir <- '
        ...
        # direct effect
        b_useful ~ bc2*b_anx + bc3*b_facilitat + b1age + b1gender + b1media + si*b_socinf
        b_ease ~  ba2*b_anx + ba3*b_facilitat + b1age + b1gender + b1media + si*b_socinf
...
fit_si_dir <- sem(model = model_si_dir, data = longer, cluster = "id")
summary(fit_si_dir, std=T, fit.measures = TRUE)
...
## H1: Cognitive, affective, and contextual factors are related to the time students spend using NLP chatbots on average. Specifcially, there is a) a positive relationship of experience with NLP chatbots, b) a negative relationship of NLP chatbot anxiety, and c) a positive relationship of social influence with the time students spend using NLP chatbots. 
...
model <- '
        ...
        # direct effect
        b_timespent ~ b_experience + b_anx +b_socinf + b1age + b1gender + b1media
    '
...
## H1: The relationship between daily perceived ease of use and the daily time spent using NLP chatbots is mediated through daily perceived usefulness of NLP chatbots
...
model <- '
        ...
        # direct effect
        w_timespent ~ wc*w_ease + gender_binary + b1age + b1media
        
        # mediator
        w_useful ~ wa*w_ease
        w_timespent ~ wb*w_useful
        ...
        # direct effect
        b_timespent ~ bc*b_ease + gender_binary + b1age + b1media
        
        # mediator
        b_useful ~ ba*b_ease
        b_timespent ~ bb*b_useful
...
model_formula <- bf(tooluse_numahead ~ set + useful.CWC + useful.pmean + ease.CWC + ease.pmean +
...
longer_comp_nona <- longer_comp_nona %>% mutate(tooluse_numahead = case_when(
  tooluse_numahead == 1 ~ 0,
  tooluse_numahead == 2 ~ 1))
                                                
# Define the model formula
model_formula <- tooluse_numahead ~ set + useful.CWC + useful.pmean + ease.CWC + ease.pmean +
                 b1anx.CGM + b1media.CGM + 
                 b1extraversion.CGM + b1agreeableness.CGM + 
                 b1conscientiousness.CGM + b1neuroticism.CGM + b1openness.CGM + 
...
longer_comp_nona %>% dplyr::filter(tooluse_numahead == 1)
```
...
```{r}
longer %>% select(lead.timespent, id, used, timespent)
```
...
longer_comp_nona_num <- dplyr::select_if(longer_comp_nona, is.numeric)

# Prepare a vector of all potential moderator variables

moderators <- grep("gmean", names(longer_comp_nona_num), invert = TRUE, value = TRUE)
moderators <- grep("^id$", moderators, invert = TRUE, value = TRUE)
moderators <- grep("^set$", moderators, invert = TRUE, value = TRUE)
moderators <- grep("ease", moderators, invert = TRUE, value = TRUE)
moderators <- grep("training", moderators, invert = TRUE, value = TRUE)
moderators <- grep("strain", moderators, invert = TRUE, value = TRUE)
moderators <- grep("^no", moderators, invert = TRUE, value = TRUE)

moderators <- grep("CWC|CGM", moderators, invert = FALSE, value = TRUE)


# Initialize an empty list to store significant moderator names
significant_moderators <- list()

# Define the static part of the formula

base_formula <- "timespent ~ set + 
  useful.CWC + useful.pmean + ease.CWC*X_MODERATOR_X + ease.pmean +
                 b1anx.CGM + b1media.CGM + 
                 b1extraversion.CGM + b1agreeableness.CGM + 
                 b1conscientiousness.CGM + b1neuroticism.CGM + b1openness.CGM + 
                 b1age.CGM + b1gender_binary + (1 | id)"
...
for (moderator in moderators) {
  ...
  # Replace placeholder with the current moderator
  model_formula <- gsub("X_MODERATOR_X", moderator, base_formula)
  
  # Fit the model
  fit_timespent <- lmer(as.formula(model_formula), 
                     data = longer_comp_nona, 
                     control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
  
  # Get the summary
  fit_summary <- summary(fit_timespent)
  
  # Extract interaction term p-value
  interaction_term <- grep("ease.CWC.*:", rownames(fit_summary$coefficients), value = TRUE)
  if (length(interaction_term) > 0) {
    interaction_p_value <- fit_summary$coefficients[interaction_term, "Pr(>|t|)"]

    # Check if the interaction is significant
    if (interaction_p_value < 0.05) {
      significant_moderators[[moderator]] <- fit_summary$coefficients[interaction_term, ]
    }
  }
}

# Print the significant moderators and their coefficients
...
# Define the model formula
model_formula <- timespentahead ~ set + 
  useful.CWC + useful.pmean + ease.CWC + ease.pmean +
                 b1anx.CGM + b1media.CGM + 
                 b1extraversion.CGM + b1agreeableness.CGM + 
                 b1conscientiousness.CGM + b1neuroticism.CGM + b1openness.CGM + 
...
# Fit the model using lmer from lme4
fit_timespentahead <- lmer(model_formula, 
                      data = longer_comp_nona, 
                      control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
...
model_formula <- ease ~ set + b1receivetr.CGM + useful.CWC +
                  useful.pmean +
                 b1anx.CGM + b1media.CGM + 
                 b1extraversion.CGM + b1agreeableness.CGM + 
                 b1conscientiousness.CGM + b1neuroticism.CGM + b1openness.CGM + 
...
interact_plot(model = fit_strain, pred = ease.CWC, modx = b1receivetr.CGM)
```
...
# Define the static part of the formula
base_formula <- "strain ~ set + ease.CWC*X_MODERATOR_X + useful.CWC +
                 ease.pmean + useful.pmean +
                 b1anx.CGM + b1media.CGM + 
                 b1extraversion.CGM + b1agreeableness.CGM + 
                 b1conscientiousness.CGM + b1neuroticism.CGM + b1openness.CGM + 
...
for (moderator in moderators) {
  ...
  # Fit the model
  fit_strain <- lmer(as.formula(model_formula), 
                     data = longer_comp_nona, 
                     control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
  
  # Get the summary
  fit_summary <- summary(fit_strain)
...
predictors <- grep("gmean", names(longer_comp_nona_num), invert = TRUE, value = TRUE)
predictors <- grep("^id$", predictors, invert = TRUE, value = TRUE)
predictors <- grep("^set$", predictors, invert = TRUE, value = TRUE)
predictors <- grep("ease", predictors, invert = TRUE, value = TRUE)
predictors <- grep("training", predictors, invert = TRUE, value = TRUE)
predictors <- grep("strain", predictors, invert = TRUE, value = TRUE)
predictors <- grep("^no", predictors, invert = TRUE, value = TRUE)

predictors <- grep("CWC|CGM", predictors, invert = FALSE, value = TRUE)
...
# Define the static part of the formula
base_formula <- "ease ~ set + X_MODERATOR_X + useful.CWC +
                 useful.pmean +
                 b1anx.CGM + b1media.CGM + 
                 b1extraversion.CGM + b1agreeableness.CGM + 
                 b1conscientiousness.CGM + b1neuroticism.CGM + b1openness.CGM + 
...
for (predictor in predictors) {
  ...
  # Replace placeholder with the current moderator
  model_formula <- gsub("X_MODERATOR_X", predictor, base_formula)
...
# Define the model formula
model_formula <- strainahead ~ set + ease.CWC + useful.CWC +
                 ease.pmean + useful.pmean +
                 b1anx.CGM + b1media.CGM + 
                 b1extraversion.CGM + b1agreeableness.CGM + 
                 b1conscientiousness.CGM + b1neuroticism.CGM + b1openness.CGM + 
...
# Fit the model using lmer from lme4
fit_strainahead <- lmer(model_formula, 
                   data = longer_comp_nona, 
                   control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)))
...
library(interactions)
interact_plot(model = fit_strainahead, pred = ease.CWC, modx = b1receivetr.CGM)
```
...
model_formula <- bf(ease.CWC ~ b1experience.CGM +
                      b1anx.CGM + b1media.CGM + 
                      b1extraversion.CGM + b1agreeableness.CGM + 
                      b1conscientiousness.CGM + b1neuroticism.CGM + b1openness.CGM + 
                      b1age.CGM + b1gender_binary + (|id))  
...
longer <- semTools::indProd(longer, var1=33:38, var2=105, match = FALSE, meanC = TRUE, residualC = FALSE, doubleMC = TRUE, namesProd = NULL)
...
model <- '
        ...
        w_timespent =~ timespent 
        timespent~~0*timespent
        w_intusefulgender =~ useful_1.gender_binary + useful_2.gender_binary + useful_3.gender_binary + useful_4.gender_binary + useful_5.gender_binary + useful_6.gender_binary
        
        w_useful ~ wb1*w_ease
        w_timespent ~ w_useful + wb2*w_intusefulgender
        ...
        b_media =~ b1media 
        b1media~~0*b1media
        b_intusefulgender =~ useful_1.gender_binary + useful_2.gender_binary + useful_3.gender_binary + useful_4.gender_binary + useful_5.gender_binary + useful_6.gender_binary
        
              # direct effect
                 b_timespent ~ bc*b_media
                 b_timespent ~ b_useful + bb2*b_intusefulgender
               # mediators
                 b_ease ~ ba1*b_media
                 b_useful ~ bb1*b_ease
               # indirect effect (a*b)
...
model <- '
        ...
        w_ease =~ ease_1 + ease_2 + ease_3 + ease_4 + ease_5 + ease_6
        w_useful =~ useful_1 + useful_2 + useful_3 + useful_4 + useful_5 + useful_6
        w_reliable =~ reliable_1 + reliable_2 + reliable_3
        w_timespent =~ timespent 
        timespent~~0*timespent
        
        w_ease ~ wa*w_reliable
        w_useful ~ wb1*w_ease
        w_timespent ~ wb2*w_useful
        w_timespent ~ wc*w_reliable
        ...
        b_timespent =~ timespent 
        timespent~~0*timespent
        b_reliable =~ reliable_1 + reliable_2 + reliable_3
        
              # direct effect
                 b_timespent ~ bc*b_reliable
                 b_timespent ~ bb2*b_useful
               # mediators
                 b_ease ~ ba*b_reliable
                 b_useful ~ bb1*b_ease
               # indirect effect (a*b)
                 babb := ba*bb1*bb2
                 wawb := wa*wb1*wb2
               # total effect
                 btotal := bc + (ba*bb1*bb2)
                 wtotal := bc + (wa*wb1*wb2)
    '
...
model <- '
        ...
        b_timespent =~ timespent 
        timespent~~0*timespent
        b_facil =~ facilitat_1 + facilitat_2 + facilitat_3 + facilitat_4
        
              # direct effect
                 b_timespent ~ bc*b_facil
                 b_timespent ~ bb2*b_useful
               # mediators
                 b_ease ~ ba1*b_facil
                 b_useful ~ bb1*b_ease
...
model <- '
        ...
        b_timespent =~ timespent 
        timespent~~0*timespent
        b_receivetr =~ b1receivetr_1 
        b1receivetr_1~~0*b1receivetr_1
...
model <- '
        level: 1
        w_ease =~ ease_1 + ease_2 + ease_3 + ease_4 + ease_5 + ease_6
        w_useful =~ lead.useful_1 + lead.useful_2 + lead.useful_3 + lead.useful_4 + lead.useful_5 + lead.useful_6
            
        w_useful ~ wb*w_ease
               
        level: 2
        b_ease =~ ease_1 + ease_2 + ease_3 + ease_4 + ease_5 + ease_6
        b_useful =~ lead.useful_1 + lead.useful_2 + lead.useful_3 + lead.useful_4 + lead.useful_5 + lead.useful_6
        b_train =~ b1receivetr 
        b_facilitat =~ facilitat_1 + facilitat_2 + facilitat_3 + facilitat_4 
              # direct effect
                 b_useful ~ bc*b_facilitat
               # mediator
                 b_ease ~ ba*b_facilitat
                 b_useful ~ bb*b_ease
               # indirect effect (a*b)
                 babb := ba*bb
                 bawb := ba*wb
...
model <- '
    ...
    fit <- lavaan::sem(model = model, data = longer, cluster = "id")
    summary(fit, std=T)
...
strain_mod <- bf(strain.pmean ~ ease.CWC + ease.pmean + useful.CWC + useful.pmean +
                      b1experience.CGM +
                      b1anx.CGM + b1media.CGM + 
                      b1extraversion.CGM + b1agreeableness.CGM + 
                      b1conscientiousness.CGM + b1neuroticism.CGM + b1openness.CGM + 
...
indirect_effect_ease <- 
  med_post$b_easepmean_b1age.CGM  *   # a path
   med_post$b_strainpmean_ease.pmean     # b path
...
library(nlme)
library(lme4)
selected <- longer_comp %>% dplyr::select(matches("CWC|strain|ease")) %>% names(.)
describe(longer_comp[,selected])

selected <- longer_comp %>% dplyr::select(matches("CWC|^id$|anx|strain|ease")) %>% names(.)

data1 <- longer_comp[,selected]

data1$x <- data1$b1anx.CGM
data1$m <- data1$ease.CWC 
data1$y <- data1$strain
...
mediationmodel.lmer <- lmer(formula = z ~  
                                0 + dm:b1anx.CGM + 
